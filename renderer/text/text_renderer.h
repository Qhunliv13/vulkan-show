...
public:
    ...
    // 结束文本渲染批次并渲染所有累积的文本
    // screenWidth/screenHeight: 用于NDC坐标转换的屏幕大小（在Fit模式下应该是viewport大小）
    ...
    // 添加居中文字到当前批次（不立即渲染）
    // centerX/centerY: 文本中心点坐标（在Fit模式下已经是窗口坐标，已转换）
    ...
    // 注意：此方法会立即渲染，如果要在同一帧渲染多个文本，请使用BeginTextBatch/AddTextToBatch/EndTextBatch
    ...
    // 注意：此方法会立即渲染，如果要在同一帧渲染多个文本，请使用BeginTextBatch/AddTextCenteredToBatch/EndTextBatch
    ...
    // 获取文字尺寸
    void GetTextSize(const std::string& text, float& width, float& height);
    
    // 获取文字中心相对于传入Y坐标的偏移（用于垂直居中）
    // 返回：文字中心相对于传入Y坐标的偏移量（正值表示文字中心在Y坐标下方）
    float GetTextCenterOffset(const std::string& text);
